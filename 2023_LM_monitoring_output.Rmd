...
filters <- list(
  ...
  # Country
  #list( field = "country.id", operation = "EQ", value = 686 ),
...
deals_data %>%
  ...
  mutate(
    ...
    modified_at = format(as.POSIXct(created_at), format="%Y-%m-%d"),
    fully_updated_at = format(as.POSIXct(created_at), format="%Y-%m-%d"),
  ) %>%
...
```{r include = TRUE, fig.height=40, fig.width=12}
